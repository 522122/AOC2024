...
func main() {
	...
	A, B, C, instructions := parseInput(&input)
	...
	for {
		...
		if out != -1 {
			programOutput = append(programOutput, out)
		}
	}

	programOutputStr := []string{}
	for _, o := range programOutput {
		programOutputStr = append(programOutputStr, strconv.Itoa(o))
	}

	fmt.Println(strings.Join(programOutputStr, ","))
}
